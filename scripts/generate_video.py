...
def make_image_slide_html(image_path: str, accent_color: str = '#c96442', progress_html: str = '') -> str:
    """原文图片全屏幻灯片"""
    img_url = 'file:///' + str(Path(image_path).resolve()).replace('\\', '/')
    return f'''<!DOCTYPE html>
<html><head><meta charset="UTF-8">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{margin:0;display:flex;justify-content:center;align-items:center;min-height:100vh;background:#fbf9f6}}
.page{{width:1920px;height:1080px;display:flex;flex-direction:column}}
.progress{{display:flex;height:8px;width:100%}}
.progress .seg{{height:100%}}
.slide{{flex:1;display:flex;justify-content:center;align-items:center;padding:40px}}
.slide img{{max-width:100%;max-height:100%;border-radius:24px;border:2px solid #e8e6e2;object-fit:contain;box-shadow:0 20px 60px -20px rgba(0,0,0,0.15)}}
</style></head><body>
<div class="page">
{progress_html}
<div class="slide"><img src="{img_url}" alt="原文图片" /></div>
</div></body></html>'''
...
def generate_video(daily: dict) -> str:
    ...
    for i, card in enumerate(cards):
        ...
        if screenshot_path and Path(screenshot_path).exists():
            img_slide = str(output_dir / f"news_{i+1}_img.png")
            render_png(make_image_slide_html(screenshot_path, color, progress_html), img_slide)
            # 图片帧配短静音或简短过渡音
            silence_audio = str(output_dir / f"news_{i+1}_silence.mp3")
            subprocess.run(['ffmpeg', '-y', '-f', 'lavfi', '-i', 'anullsrc=r=24000:cl=mono', '-t', '2', '-q:a', '9', '-acodec', 'libmp3lame', silence_audio], capture_output=True)
            segments.append((img_slide, silence_audio, 2.5))
